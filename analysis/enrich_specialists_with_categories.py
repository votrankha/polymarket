...
def main():
    ...
    # Enrich with market categories and performance
    enriched = []
    for s in specialists:
        addr = s['address']

        # Get top markets with questions
        top_markets_sql = """
        ...
        for m in markets_raw:
            ...
            mkt_row = q("SELECT question FROM markets WHERE condition_id = ?", (m['market_id'],))
            question = mkt_row[0]['question'] if mkt_row else ''
        ...
        perf_sql = """
        SELECT realized_pnl FROM closed_positions WHERE address = ?
        """
        pos = q(perf_sql, (addr,))
        if pos:
            wins = []
            ...
            perf = {'wr': 0, 'kelly': 0, 'total': 0, 'wins': 0, 'losses': 0, 'avg_win': 0, 'avg_loss': 0}

        # Check promoted?
        promoted_sql = """
        SELECT 1 FROM wallet_snapshots
        WHERE address = ? AND ts = (SELECT MAX(ts) FROM wallet_snapshots WHERE ts >= (strftime('%s','now')-86400*7))
        """
        ...
        enriched.append({
            **s,
            ...
            'secondary_categories': [cat for cat, _ in categories.most_common(2) if cat != categories.most_common(1)[0][0]],
            'performance': perf,
            'promoted': promoted,
        })
